...
function RegisterEmployee() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await registerEmployeeRequest(formData);
      ...
      if (!response.ok) {
        ...
        if (errorData.message && errorData.message.includes('Email:')) {
          ...
          handleError('Blad rejestracji pracownika... sprawdz konsole');
        }
      } else {
        navigate('/employees');
      }
  ...
  return (
    <>
      <GreenBackground height={"80vh"} marginTop={"2vh"}>
        <Header>{dictionary.registerPage.registerEmpTitle}</Header>
          ...
          <form onSubmit={handleSubmit}>
            <Box sx={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "2vh", }}>
              ...
              <CustomInput
                ...
                onChange={handleChange}
                error={positionError}
                helperText={positionError ? dictionary.registerPage.positionError : ""}
                required
                size="small"
              >
