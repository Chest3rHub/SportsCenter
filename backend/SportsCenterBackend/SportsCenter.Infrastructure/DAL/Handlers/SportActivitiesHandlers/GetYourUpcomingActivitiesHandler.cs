...
{
    internal class GetYourUpcomingActivitiesHandler : IRequestHandler<GetYourUpcomingActivities, IEnumerable<YourSportActivityDto>>
    {
        private readonly SportsCenterDbContext _dbContext;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public GetYourUpcomingActivitiesHandler(SportsCenterDbContext dbContext, IHttpContextAccessor httpContextAccessor)
        {
            _dbContext = dbContext;
            _httpContextAccessor = httpContextAccessor;
        }

        public async Task<IEnumerable<YourSportActivityDto>> Handle(GetYourUpcomingActivities request, CancellationToken cancellationToken)
        {
            ...
            var sportActivities = await _dbContext.InstancjaZajecKlients
                ...
                .Where(activity => activity.DateOfActivity >= currentDate)
